...
> 关联文档:[项目手册.md §4.5-4.6 / §7-§8](./项目手册.md) · [安全设计.md](./安全设计.md) · [架构设计.md](./架构设计.md)
>
...
| 目标 | 说明 |
|---|---|
| 私有信封格式 | 通用工具(hashcat / John)不识别,见 [安全设计.md §5](./安全设计.md#5-自定义信封格式设计哲学) |
...
> **扩展映射**:JSON 文件格式字段 → 二进制存储字段的对应关系见 [平台模板参考.md §9.1](./平台模板参考.md#91-与-okv_formatmd-领域模型的对齐)。二进制存储仅保留运行时必需字段(Key / Kind / Sensitive / Required / DefaultMask / Validation),UI 展示字段(Label / Placeholder / Description / Group 等)从模板文件实时加载。
...
| **0.2** | **2026-06-24** | **顶部 cross-ref 更新(PRD.md → 项目手册.md);二进制格式与字段定义不变** |
