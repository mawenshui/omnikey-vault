...
| 文档版本 | 日期 | 作者 | 状态 |
|---|---|---|---|
| **2.3** | **2026-07-23** | **Sisyphus** | **v2.3.8 发布：70 个平台模板、浏览器扩展、全局热键、命令面板、791/791 tests** |
| 1.7 | 2026-07-15 | Sisyphus | v1.7 发布:50 平台模板 + 安装包 + 应用图标,578/578 tests |
...
> 关联文档:[项目手册.md](./项目手册.md) · [安全设计.md](./安全设计.md) · [文件格式规范.md](./文件格式规范.md) · [内部接口规范.md](./内部接口规范.md)
...
- **外部审计人员**:验证架构是否支撑 [项目手册.md §7 / 安全设计.md §2](./项目手册.md#7-安全模型) 中的安全不变量。
...
**决策**:为了 [项目手册.md §18 远期规划](./项目手册.md#18-远期v1x需-v1-成功验证后再决定) 的 macOS / Linux 扩展路径,选择 Avalonia。短期成本:设计师资源略少;长期收益:代码复用。
...
**决策**:Sodium.Core 的抗误用 API 更契合 [项目手册.md §7 / 安全设计.md §3](./项目手册.md#7-安全模型) 的"密码学可审计"目标。BouncyCastle 留作备选,仅在合规场景需要 FIPS 认证时切换。
...
> **v0.2 落地状态**:Views/ 下 24 个 XAML 视图 + code-behind 已实现(v0.1:7 个 + v0.2:17 个新增)。详见 [项目手册.md §17.2](./项目手册.md#172-v02已交付--多-profile--dev-备份--同步)。

### 4.2 内部 CLI 模块(非用户面向)

> 该模块仅为集成测试 / CI 自动化 / 调试提供入口;**不是产品用户接口**(产品用户接口 = GUI,见 §1.2 架构目标)。详见 [内部接口规范.md](./内部接口规范.md)。
...
- **不**承诺长期向后兼容(接口可能随内部重构变化)。详见 [内部接口规范.md](./内部接口规范.md)。
...
└── publish/                                  # 编译产物(详见 [编译打包指南.md](./编译打包指南.md))
...
- **崩溃**:未处理异常过滤器在写日志后主动清零敏感内存并吞掉转储(见 [安全设计.md §6](./安全设计.md#6-内存安全))。
...
> **v0.2 实际状态**:`okv` 二进制以**框架依赖(FDD)**形态发布,目标 RID = `win-x64`(详见 [编译打包指南.md §5](./编译打包指南.md#5-打包--发布-publishing--packaging))。v1.0 落地以下三种分发形态。
...
- **内部 CLI 模式**(`okv <subcommand>`):可运行于任意有 .NET 8 Runtime 的环境,包括无头 Linux / Windows Server Core / Docker(详见 [编译打包指南.md §6 容器化](./编译打包指南.md#6-容器化可选))。
...
- **状态**:Accepted
- **日期**:2026-06-17
- **背景**:[项目手册.md §1.4](./项目手册.md#14-核心价值主张) 要求"通用工具不识别"。KDBX4 被 hashcat mode 13400 专门支持;Bitwarden 格式公开。
...
- **状态**:Accepted
- **日期**:2026-06-17
- **背景**:[项目手册.md §4.4](./项目手册.md#44-多设备同步) 要求零运维。自建同步协议需服务端,违反"本地优先"。
...
详见 [开发路线图.md](./开发路线图.md) 各里程碑的验收标准。

---

## 13. 性能与容量

### 13.1 性能目标(对应 [项目手册.md §5 非功能需求](./项目手册.md#5-非功能需求))
...
参见 [项目手册.md 附录 A](./项目手册.md#附录-a--术语表)。
...
| 0.2 | 2026-06-24 | 适配 v0.2 实际状态:GUI 已落地(`src/OmniKeyVault.Cli/Gui/Views/` 24 个 XAML);CLI 重新定位为内部接口(详见 [内部接口规范.md](./内部接口规范.md));二进制名 `OmniKeyVault.exe` → `okv.exe`;cross-ref 全面更新(PRD.md → 项目手册.md);新增 §2.2.2 入口层 / §4.1 UI 模块清单 / §4.2 内部 CLI 模块;ADR-006 重新表述 |
