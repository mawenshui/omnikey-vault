...
| 文档版本 | 日期 | 作者 | 状态 |
|---|---|---|---|
| **2.3** | **2026-07-23** | **Sisyphus** | **v2.3.8 发布：功能全面交付，转向体验打磨与跨平台扩展，791/791 tests** |
| 1.4 | 2026-07-15 | Sisyphus | v1.7:50 平台模板 + 安装包 + 应用图标,578/578 tests |
...
> 关联文档:[项目手册.md §16-18 路线图](./项目手册.md#16-已交付里程碑) · [架构设计.md](./架构设计.md) · [安全设计.md](./安全设计.md) · [测试报告.md](./测试报告.md)
>
> 本文档把 [项目手册.md §16-18 路线图](./项目手册.md#16-已交付里程碑) 拆解为可执行的 Sprint 任务,服务于开发团队日常协作。**v0.1 / v0.2 已交付,本文件记录实际完成情况**;v0.3+ 任务为下一阶段规划。
...
1. 记录 v0.1 / v0.2 已交付的实际 Sprint 任务与产出(对照 [测试报告.md](./测试报告.md))。
...
### 3.1 目标(对应 [项目手册.md §16.1 v0.1 MVP](./项目手册.md#161-v01-mvp已交付))
...
> **v0.1 已交付**(2026-06-22,170/170 测试通过)。实际状态详见 [测试报告.md §1 / §2](./测试报告.md)。
...
| S1-T2 | `ICryptoProvider` 接口 + LibSodiumCryptoProvider 实现 | 后端 | CryptoProvider 单元测试 | INV-01 ~ INV-04 通过(见 [安全设计.md §10](./安全设计.md#10-密码学不变量)) | 3d |
...
| ID | 任务 | 负责 | 产出 | 验收标准 | 预估 |
|---|---|---|---|---|---|
| S2-T1 | Avalonia 项目脚手架 + 主窗口布局 | UI | MainWindow.axaml | 主窗口布局符合 [项目手册.md §10.1](./项目手册.md#101-总体布局) | 2d |
...
| S2-T7 | 5 个平台模板 JSON + 模板加载器 | 全栈 | templates/*.json | 5 模板可创建条目,字段定义符合 [平台模板参考.md §3](./平台模板参考.md#3-v01-mvp-模板5-个完整-json) | 1d |
| S2-T8 | 剪贴板服务 + 8 秒自动清空 | 全栈 + UI | ClipboardService | UI-INT-01 通过 | 1d |
| S2-T9 | Bitwarden JSON 导入器 | 全栈 | BitwardenImporter | 导入 100 条目无错 | 2d |
| S2-T10 | 状态栏 + 同步状态占位(无 watcher) | UI | StatusBar | 布局符合 [项目手册.md §10.4](./项目手册.md#104-状态栏底) | 1d |
...
| 安全 | INV-01 ~ INV-10 全部通过(见 [安全设计.md §10](./安全设计.md#10-密码学不变量)) |
| 格式 | FMT-RW-01 ~ 05 + FMT-INTEG-01 ~ 04 + FMT-RECOV-01 ~ 03 通过 |
| UI | UI-AUTO-01 ~ 02 + UI-INT-01 ~ 02 通过 |
| 性能 | 解锁 ≤1.5s + 启动 ≤2s + 1000 条目加载 ≤2s |
| 文档 | [架构设计.md](./架构设计.md) / [安全设计.md](./安全设计.md) / [文件格式规范.md](./文件格式规范.md) / [项目手册.md §10](./项目手册.md#10-主窗口布局) 同步更新 |
...
> **v0.2 已交付**(2026-06-23,357/357 测试通过)。实际状态详见 [测试报告.md §1 / §2](./测试报告.md)。本节记录实际完成的 Sprint 任务;**GUI 任务(S3-T2 / S4-T5 / S4-T8)在 v0.2 中一并落地**(`src/OmniKeyVault.Cli/Gui/Views/` 24 个 XAML 视图)。

### 4.1 目标(对应 [项目手册.md §16.2 v0.2](./项目手册.md#162-v02已交付--多-profile--dev-备份--同步))
...
| 类别 | 标准 | 状态 |
|---|---|---|
| 功能 | [项目手册.md §16.2 v0.2 全部目标](./项目手册.md#162-v02已交付--多-profile--dev-备份--同步) | ✅ 通过 |
...
### 5.1 目标(对应 [项目手册.md §16.3 v0.3](./项目手册.md#163-v03已交付--富功能--国际化))

- KDBX 导入(实际落地为 KeePass 2.x XML,见 [内部接口规范.md §2.2 `import --format kdbx-xml`](./内部接口规范.md))
- 全文搜索 + 字段级搜索
- 附件 Blob 加密存储
- 国际化(英文)
...
| 类别 | 标准 | 状态 |
|---|---|---|
| 内部 CLI(已交付) | [内部接口规范.md §3 退出码表](./内部接口规范.md#3-退出码权威表) 全部正确 + §7.1-7.4 安全测试通过 | ✅ 通过(74 v0.2 CLI tests + 14 v1 CLI tests) |
...
### 6.1 目标(对应 [项目手册.md §16.4 v0.4](./项目手册.md#164-v04已交付--一键轮换--历史--性能))
...
| S8-T6 | 安全测试清单全量执行(见 [安全设计.md §11](./安全设计.md#11-安全审计清单)) | QA | 安全报告 | 全部清单项通过(8/8 落地的不变量) | ✅ 已交付(v0.2 测试报告 + 增量 v0.3/v0.4) |
...
| 安全 | 安全设计.md §11 清单全通过 | ✅ 通过(8/8 落地不变量 + 增量覆盖) |
...
### 7.1 目标(对应 [项目手册.md §17 v1.0 RC](./项目手册.md#17-v10-候选下一步--公开发布准备))
...
| S9-T3 | 密码学白皮书(基于 [安全设计.md §3-§4](./安全设计.md) 扩展) | Tech Lead | [CryptoWhitepaper.md](./CryptoWhitepaper.md) | 外部可理解 | ⏸ 待开始 |
| S9-T4 | 同步协议规范独立文档(基于 [架构设计.md §10](./架构设计.md) 扩展) | 后端 | [SyncProtocol.md](./SyncProtocol.md) | 独立可读 | ⏸ 待开始 |
| S9-T5 | 威胁模型文档独立化(基于 [安全设计.md §2](./安全设计.md) 扩展,STRIDE) | Tech Lead | [ThreatModel.md](./ThreatModel.md) | STRIDE 完整覆盖 | ⏸ 待开始 |
| S9-T6 | 用户手册 + 快速入门(从 [项目手册.md](./项目手册.md) 提取面向新用户版本) | UI / QA | [UserGuide.md](./UserGuide.md) | 新用户 5 分钟上手 | ⏸ 待开始 |
...
| S10-T3 | 单文件可执行 + Portable ZIP 构建(基于 [编译打包指南.md §5](./编译打包指南.md)) | 全栈 | 发行包 | 3 种形态可下载 | ⏸ 待开始 |
...
### 12.3 项目结构(对应 [架构设计.md §7.2](./架构设计.md#72-解决方案结构))
...
- `INV-XX`:密码学不变量(见 [安全设计.md §10](./安全设计.md#10-密码学不变量))。
- `FMT-XX-XX`:格式测试(见 [文件格式规范.md §15](./文件格式规范.md#15-测试用例要求))。
- `UI-XX-XX`:UI 测试(见 [项目手册.md §15.8 + 测试报告.md](./项目手册.md#158-不变测试))。
- `CLI-XX-XX`:内部 CLI 测试(见 [内部接口规范.md §9](./内部接口规范.md#9-测试用例要求))。
- `SEC-XX-XX`:安全威胁测试(见 [安全设计.md §2](./安全设计.md#21-我们防御的威胁))。
...
| 0.2 | 2026-06-24 | v0.1 / v0.2 标记为已交付(S3-T2 / S4-T5 / S4-T8 等 GUI 任务在 v0.2 落地);v0.3 Sprint 5 任务重组:CLI 任务(S5-T1 ~ T5)取消(v0.2 提前交付),改为 KDBX + GUI 端到端测试 + watcher 完整实现;§12 命令更新(GUI 为默认启动);cross-ref 全面更新(PRD.md → 项目手册.md,UI_UX_SPEC.md → 项目手册.md,CLI_SPEC.md → 内部接口规范.md) |
