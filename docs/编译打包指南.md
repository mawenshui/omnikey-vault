...
| 文档版本 | 日期 | 作者 | 状态 |
|---|---|---|---|
| 1.5 | 2026-07-15 | Sisyphus | v1.7 发布:50 平台模板 + Inno Setup 安装包 + 应用图标 + 578/578 tests |
...
> 本文档面向开发者、CI 维护者和发布工程师,覆盖 OmniKey Vault(`okv.exe` / `okv`)从源码到分发的完整生命周期。**用户面向入口是 GUI 桌面应用**(`okv` 无参数启动);**内部 / CI 自动化入口是 CLI**(`okv <subcommand>`,详见 [内部接口规范.md](./内部接口规范.md))。所有命令在 Windows / Linux / macOS 上等价(以 PowerShell 与 Bash 双向给出)。
...
tests/
└── OmniKeyVault.Tests/              # xUnit + FluentAssertions,457 个测试(详见 [测试报告.md](测试报告.md))
...
耗时约 17-19 秒(测试模式 Argon2id `t=3, m=32 MiB`)。详细覆盖见 [测试报告.md](测试报告.md)。
...
> `okv.exe` 是单一可执行文件;无参数 = GUI 主程序,有参数 = 内部 CLI(详见 [内部接口规范.md](./内部接口规范.md))。
...
`--` 是必需的:`dotnet run` 用它把后续参数转发给程序(而非 dotnet 自身)。CLI 命令参考见 [内部接口规范.md §5](./内部接口规范.md#5-关键命令速查)。
...
### 5.8 Inno Setup 安装包（v1.7 引入，推荐用户分发）

> 目标机器**不需要**任何 .NET 运行时。安装包自动配置开始菜单、桌面快捷方式、`.okv` 文件关联。

**先决条件**:
- [Inno Setup 6](https://jrsoftware.org/isdl.php) 安装在 `C:\Program Files (x86)\Inno Setup 6\`

**步骤**:

```powershell
# 1. 先执行自包含发布（见 §5.2 或 §5.3）
dotnet publish src/OmniKeyVault.Cli/OmniKeyVault.Cli.csproj `
  -c Release -r win-x64 --self-contained true `
  -o ./publish/win-x64
...
**产物**:`installer/installer_output/OmniKeyVault-Setup-{version}.exe`

**安装包特性**:
- LZMA2 Ultra64 压缩（~31 MB 安装包 vs ~42 MB 原始文件）
- 开始菜单快捷方式 + 桌面快捷方式（可选）
- `.okv` 文件关联（可选）
- 中文 + 英文安装界面
- 完整卸载（删除程序文件，保留用户数据）
- 内嵌应用图标（`okv.exe` + 安装包 + 卸载程序）

**应用图标**: 图标由 `tools/generate_icon.py`（Pillow）生成，保存在 `images/okv-icon.ico`（7 种尺寸：16/24/32/48/64/128/256）。重新生成:
...
Compress-Archive -Path publish\win-x64\* `
  -DestinationPath publish\OmniKeyVault-{version}-win-x64.zip -Force
```
...
| ~~GUI 启动器~~ | ~~不存在(AvAvalonia 推迟到 v0.2.1)~~ | **v0.2 已落地**:`src/OmniKeyVault.Cli/Gui/Views/` 下 24 个 Avalonia 视图(详见 [测试报告.md §2.1 S3-T2/S4-T5/S4-T8 状态更新](./测试报告.md));`okv`(无参数)直接启动 GUI |
...
| ~~GUI 发布~~ | **v0.2 已落地**:`okv`(无参数)直接启动 Avalonia 11 桌面应用;`publish/win-x64-fd/` 产物为框架依赖版。详见 [项目手册.md §17.2](./项目手册.md#172-v02已交付--多-profile--dev-备份--同步)。 |
...
- [项目手册.md](项目手册.md) — 用户面向项目手册(产品 / UI / UX / 路线图)
- [开发路线图.md](开发路线图.md) — Sprint 任务分解与里程碑
- [架构设计.md](架构设计.md) — 分层与依赖方向
- [内部接口规范.md](内部接口规范.md) — 内部 CLI 模式(集成测试 / CI 自动化入口)规范
- [文件格式规范.md](文件格式规范.md) — `.okv` / `.okv.dev` 二进制规范
- [安全设计.md](安全设计.md) — 威胁模型与密码学不变量
- [测试报告.md](测试报告.md) — v0.2 测试覆盖与偏差
- [平台模板参考.md](平台模板参考.md) — 11 个模板字段定义
...
**文档结束**。如有疑问,优先查阅 [测试报告.md](测试报告.md) §9(已知偏差)与 [架构设计.md](架构设计.md) §10(ADR)。v0.2.1 引入 CI 后,本文档将由自动化生成的 release notes 补充。
...
| 0.3 | 2026-06-24 | §1 / §4 / §9 / §11 / §12 全部更新:GUI 作为主程序(无参数启动);内部 CLI 仅作集成测试 / CI 入口(详见 [内部接口规范.md](./内部接口规范.md));§9 偏差表中"GUI 启动器"项划掉(已落地,24 个 Avalonia 视图) |
